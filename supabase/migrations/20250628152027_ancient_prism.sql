...
-- RLS policies for redemption_requests - add IF NOT EXISTS to avoid errors
...
CREATE POLICY IF NOT EXISTS "Users can create own redemption requests"
  ON redemption_requests
  FOR INSERT
  TO authenticated
  WITH CHECK (user_id = (auth.jwt() ->> 'sub') OR user_id = auth.jwt() ->> 'email');

CREATE POLICY IF NOT EXISTS "Users can view own redemption requests"
  ON redemption_requests
  FOR SELECT
  TO authenticated
  USING (user_id = (auth.jwt() ->> 'sub') OR user_id = auth.jwt() ->> 'email');

CREATE POLICY IF NOT EXISTS "Admins can manage all redemption requests"
  ON redemption_requests
  FOR ALL
  TO authenticated
  USING (
...
DROP FUNCTION IF EXISTS update_redemption_requests_updated_at() CASCADE;
...
DROP TRIGGER IF EXISTS update_redemption_requests_updated_at ON redemption_requests;
...
DROP FUNCTION IF EXISTS log_redemption_activity() CASCADE;
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (
      ...
      'Requested redemption of ' || NEW.points_used || ' points for ' || NEW.cash_value || ' ' || NEW.redemption_type,
      jsonb_build_object(
...
DROP TRIGGER IF EXISTS redemption_activity_trigger ON redemption_requests;
